...
static char filename[] = "c:\\lab28\\dict4.txt";
...
void BubbleSortStrings() {
	for (int i = 0; i < n - 1; i++) {
		for (int j = (n - 1); j > i; j--) { // перебор с конца 
			// если текущий элемент меньше предыдущего
			if (strcmp(a[j - 1], a[j]) > 0) { // сравнение строк 
				// меняем их местами
				char tmp[MAX_LEN_WORD + 1];// создается переменная для обмена значений
				strcpy(tmp, a[j - 1]);// вторую строку копирует в первую
				strcpy(a[j - 1], a[j]);
				strcpy(a[j], tmp);
			}
		}
